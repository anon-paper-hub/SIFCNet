...
def parse_options(is_train=True):
    ...
    opt_path = 'SIFCNet/Options/SIFCNet_LOL_v1.yml'
    if not os.path.exists(opt_path):
        raise FileNotFoundError(f"Cannot find the options file: {opt_path}")

    with open(opt_path, mode='r', encoding='utf-8', errors='ignore') as f:
        ...
        while train_data is not None:
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                model.validation(val_loader, current_iter, tb_logger,
                                 opt['val']['save_img'], rgb2bgr=True, use_image=True)

                # 取出指标
                psnr_val = model.metric_results.get('psnr', None)
                ssim_val = model.metric_results.get('ssim', None)
                log_vars = model.get_current_log()
                l_pix = log_vars.get('l_pix', None)

                # 写入 csv
                logger_metric = get_root_logger(logger_name='metric')
                metric_str = f'{current_iter}'
                if psnr_val is not None:
                    metric_str += f',{psnr_val:.6f}'
                if ssim_val is not None:
                    metric_str += f',{ssim_val:.6f}'
                if l_pix is not None:
                    metric_str += f',{l_pix:.6f}'
                logger_metric.info(metric_str)

                # 保存 best（以 psnr 为准）
                if psnr_val is not None and psnr_val > best_metric['psnr']:
                    best_metric['psnr'] = psnr_val
                    best_metric['iter'] = current_iter
                    model.save_best(best_metric)

                if tb_logger:
                    tb_logger.add_scalar(f'metrics/best_iter',
                                         best_metric['iter'], current_iter)
                    tb_logger.add_scalar(f'metrics/best_psnr',
                                         best_metric['psnr'], current_iter)
...
if __name__ == '__main__':
    ...
    torch.cuda.set_per_process_memory_fraction(0.8, device=0)  # 限制 GPU 使用率
