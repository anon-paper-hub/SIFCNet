...
class ImageCleanModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('color_opt'):
            ...
            self.cri_color = cri_color_cls(**train_opt['color_opt']).to(self.device)
        else:
            self.cri_color = None
        # 加入 SSIM 损失
        ...
        if train_opt.get('semantic_opt'):
            sem_type = train_opt['semantic_opt'].pop('type')
            cri_sem_cls = getattr(loss_module, sem_type)
            self.cri_sem = cri_sem_cls(**train_opt['semantic_opt']).to(self.device)
        else:
            self.cri_sem = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            for pred in preds:
                ...
                if self.cri_sem:
                    l_sem += self.cri_sem(pred, self.gt)
            ...
            if self.cri_sem:
                loss_dict['l_sem'] = l_sem
            # 总 loss
            l_total = l_pix + l_color + l_ssim + l_sem
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger,
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            print("Processing:", val_data['lq_path'][0])
            test()
            ...
            #我加的-------------------
            # 获取保存路径
            save_dir = self.opt['val'].get('save_img_path', 'results/LOLv1xg')
            os.makedirs(save_dir, exist_ok=True)
            # 获取当前图像所属的数据集名称或路径
            dataset_name = self.opt['datasets']['val']['name']  # 假设从yaml中获取数据集名称
            # 判断是否是测试集，如果是测试集则保存图像
            if save_img and dataset_name == 'ValSet':  # 确保这里的名字是你在yaml文件中指定的验证集的名字
                save_img_path = osp.join(save_dir, f'{img_name}.png')
            #save_gt_img_path = osp.join(save_dir, f'{img_name}_gt.png')
#结束----------------
