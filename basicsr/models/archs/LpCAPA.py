...
class EnhancedLpExtractor(nn.Module):
    def __init__(self, in_channels=3, mid_channels=32):
        ...
        self.conv_out = nn.Conv2d(in_channels, 1, kernel_size=1)  # 输出单通道Lp

    def forward(self, x):
        low = self.low(x)  # 小波低频 [B, 3, H/2, W/2]
        low_up = F.interpolate(low, size=x.shape[2:], mode='bilinear', align_corners=False)  # 上采样

        low_ca = self.ca(low_up)
        low_pa = self.pa(low_ca)
        lp = self.conv_out(low_pa)  # 输出单通道 Lp
...
class Illumination_Estimator(nn.Module):
    def __init__(
        ...
        # === 添加：低频亮度提取模块 ===
        self.lp_extractor = EnhancedLpExtractor()  # ⬅️ 替换为增强亮度模块
    ...
    def forward(self, img):
        ...
        # === 新版 Lp：小波 + 注意力增强 ===
        enhanced_lp = self.lp_extractor(img)  # [B, 1, H, W]

        input = torch.cat([img, enhanced_lp], dim=1)  # 将原图I和灰度图Lp在通道维度上拼接（由3通道变为4通道）
