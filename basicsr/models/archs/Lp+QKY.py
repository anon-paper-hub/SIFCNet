...
class IG_MSA(nn.Module):  # dim输入特征的通道维度。 dim_head每个注意力头的特征维度，默认为 64 。heads注意力头的数量，默认为 8
    def __init__(
        ...
        self.dim = dim
        #滴滴滴 ✅ 新增：可学习的门控参数 gate（Sigmoid 限制在 0-1）
        self.gate = nn.Parameter(torch.zeros(heads, 1, 1))
    ...
    def forward(self, x_in, illu_fea_trans):  # illu_fea_trans：经过特征变换？？？？？、、、？的光照特征图(变换的Flu)
        ...
        # ✅ 添加门控 Y： (QK^T + gate * Y)
        gate = torch.sigmoid(self.gate)  # 限制在 0-1
        y_proj = torch.matmul(y, y.transpose(-2, -1))  # [b, heads, n, n]
        attn = attn + gate * y_proj
